...
namespace Library_Labels_Namespace
{
    public class NativeMethods
    {
        ...
        // poop we really want a send list of byte to printer...right?
        public static bool SendBytesToPrinter(string printer, byte[] data)
        {
            Log.AppendSuccess("SendBytesToPrinter");
            Int32 dwError = 0, dwWritten = 0;
            IntPtr hPrinter = new IntPtr(0);
            DOCINFOA di = new DOCINFOA();
            bool bSuccess = false; // Assume failure unless you specifically succeed.
            ...
            if (OpenPrinter(printer.Normalize(), out hPrinter, IntPtr.Zero))
            {
                ...
                if (StartDocPrinter(hPrinter, 1, di))
                {
                    ...
                    if (StartPagePrinter(hPrinter))
                    {
                        ...
                        // Call unmanaged code 
                        bSuccess = WritePrinter(hPrinter, p, data.Length, out dwWritten);
        ...
        public static bool SendFileToPrinter(string szPrinterName, string szFileName)
        {
            ...
            // Dim an array of bytes big enough to hold the file'p contents.
        ...
        public static void SendStringToPrinter(string szPrinterName, string szString)
        {
            ...
            SendBytesToPrinter(szPrinterName, a.GetBytes(szString));
        }
